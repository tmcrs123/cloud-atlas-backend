import { type AttributeValue, BatchWriteItemCommand, DynamoDBClient, PutItemCommand, QueryCommand, type QueryCommandOutput, UpdateItemCommand, type WriteRequest } from '@aws-sdk/client-dynamodb'
...
import type { CreateImageDTO, Image, UpdateImageDTO } from '../schemas/images-schema.js'
...
export class DynamoDbImagesRepository implements ImagesRepository {
  ...
  async deleteImages(atlasId: string, imageIds: string[]): Promise<void> {
    ...
    if (unprocessedImagesIds.length <= 25) {
      await sendCommand(() => this.dynamoClient.send(createBatchWriteCommand(unprocessedImagesIds, this.appConfig.configurations.imagesTableName)))
      return
    }

    while (unprocessedImagesIds.length !== 0) {
      const batch = unprocessedImagesIds.splice(0, 25)
      const command = createBatchWriteCommand(batch, this.appConfig.configurations.imagesTableName)
      await sendCommand(() => this.dynamoClient.send(command))
    }
  }
  ...
  async getImagesForMarker(atlasId: string, markerId: string): Promise<Image[]> {
    ...
    const KeyConditionExpression = 'atlasId = :atlasId AND markerId = :markerId '
  ...
  async updateImage(updatedData: UpdateImageDTO, atlasId: string, imageId: string): Promise<Image> {
